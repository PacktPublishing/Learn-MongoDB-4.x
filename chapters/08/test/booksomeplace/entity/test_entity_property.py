...
class TestProperty(unittest.TestCase) :
    ...
    testDict = {
        ...
        'partnerKey'  : 'BBBB2222',
        'propName'    : 'The Flintstone Bed and Breakfast',
        'propInfo'    : PropInfo(),
        'address'     : Location(),
        'contactName' : Name(),
        'contactInfo' : Contact(),
    ...
    def test_property_generate_property_key(self) :
        expected = 'THEF'
        key      = self.propertyFromDict.generatePropertyKey()
        actual   = key[0:4]
        self.assertEqual(expected, actual)

    def test_property_generate_property_key_short_name_adds_underscore(self) :
        self.propertyFromDict.update({'propName' : 'The'})
        expected = 'THE_'
        key      = self.propertyFromDict.generatePropertyKey()
        actual   = key[0:4]
        self.assertEqual(expected, actual)
