...
class LoanService(Base) :
    ...
    @return pymongo.cursor.Cursor : list of loan keys
    """
    def fetchLoanKeys(self) :
        return self.collection.find({},{"loanKey":1})

    """
    Get count of number of loans
    @return int : number of loan docs
    """
    def fetchLoanCount(self) :
        # for some reason this doesn't work:
        # return self.collection.count({})
        keys = self.fetchLoanKeys()
        count = 0
        for doc in keys :
            count += 1
        return count
    ...
    """
    Get the Nth loan document
    @param int skipNum : number of documents to skip before returning a single entry
    @return biglittle.entity.Loan : loan document
    """
    def fetchRandLoan(self, skipNum) :
        return self.collection.find_one({},{},skipNum)
    ...
    @return dict : result of aggregation
    """
